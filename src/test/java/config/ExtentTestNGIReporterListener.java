...
/**
 * 假如无法显示样式需要将css 和js设置成 本地的 cdn的目前失效无访问
 */
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    public static void main(String[] args) {
        String outputFolder = ExtentTestNGIReporterListener.getOutputFolder();
    ...
    private void init() {
        ...
        ExtentHtmlReporter htmlReporter = new ExtentHtmlReporter(OUTPUT_FOLDER+"\\"+format+"-" + FILE_NAME);
        htmlReporter.config().setDocumentTitle(ReportConfig.getReportName());
        htmlReporter.config().setReportName(ReportConfig.getReportName());
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                while (logIterator.hasNext()){
                    ...
                    if(details.contains(ReportConfig.getSpiltTimeAndMsg())){
                        String time = details.split(ReportConfig.getSpiltTimeAndMsg())[0];
                        log.setTimestamp(getTime(Long.valueOf(time)));
                        log.setDetails(details.substring(time.length()+ ReportConfig.getSpiltTimeAndMsg().length()));
                    }else{
